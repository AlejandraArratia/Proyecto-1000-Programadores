...
class VentanaPrincipal(QMainWindow):
    ...
    def mousePressEvent(self,event):
        ...
        if event.globalPos().y() <=10:
            ...
            self.bt_restaurar.hide()
            self.bt_maximizar.show()
    #Metodo para mover el menú lateral izquierdo
    def mover_menu(self):
        if True:
    ...
    def registrar_productos(self):
        ...
        marca=self.reg_marca.text().upper()
        precio=self.reg_precio.text().upper()
        cantidad=self.reg_cantidad.text().upper()
        if codigo!=''and nombre!=''and marca!=''and precio!='' and cantidad!='':
            self.base_datos.inserta_producto(codigo,nombre,marca,precio,cantidad)
            self.signal_registrar.setText('Productos Registrados')
            self.reg_codigo.clear()
            self.reg_nombre.clear()
            self.reg_marca.clear()
    ...
    def buscar_por_nombre_actualiza(self):
        ...
        if len(self.producto)!=0:
            ...
            self.act_marca.setText(self.producto[0][3])
    ...
    def buscar_por_nombre_eliminar(self):
        ...
        if len(producto)==0:
            self.signal_eliminacion.setText('INCORRECTO')
        else:
